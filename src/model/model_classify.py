# -*- coding: utf-8 -*-
"""
Created on Tue Oct 18 10:10:46 2016

@author: zhuxiaodong

将优惠券分为两类
1. 没有用户使用过该类优惠券，但有记录有人在这家店消费过
2. 有用户使用过该类优惠券
"""
...
coupon_distance_record = pickle.load(open('../../ccf_data/statistic_analysis/coupon_distance_record.pkl','rb'))
user_use_coupon_according_distance_record = pickle.load(open('../../ccf_data/statistic_analysis/user_use_coupon_according_distance_record.pkl','rb'))
coupon_id2 = pickle.load(open('../../ccf_data/statistic_analysis/coupon_id2.pkl','rb'))
coupon_id3 = pickle.load(open('../../ccf_data/statistic_analysis/coupon_id3.pkl','rb'))
...
for line in lines:
    ...
    if user_use_coupon_date_record.has_key(line[0]):
        coupon_num = user_use_coupon_date_record[line[0]][0] + user_use_coupon_date_record[line[0]][1]
        if coupon_num > 0:
            record.extend([user_use_coupon_date_record[line[0]][0]*1.0/coupon_num])
        else:
            record.extend([0.0])
    else:
        record.extend([0.0])
    ...
    train_data_coupon2.append(record)
csvfile.close()
...
for line in lines:
    if line[2] == 'null':
        continue
    if not coupon_id3.has_key(line[2]):
        continue
    record = []
    record.extend(line)
    '''
    ...
    train_data_coupon3.append(record)
csvfile.close()
...
pos_index = random.sample(range(len(train_data_pos)), 45000) #5w正样本训练
neg_index = random.sample(range(len(train_data_neg)), 45000) #15w负样本训练
